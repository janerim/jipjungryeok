...
설정 > 손쉬운 사용 > VoiceOver 를 켜고 확인한다.

- [ ] 다이얼에서 **위아래로 쓸어넘기면 1분씩** 시간이 바뀐다 (원형 드래그는 낭독으로 흉내낼 수 없어 따로 넣은 동작이다)
- [ ] 하단 숫자가 **버튼으로 읽히고**, 상태에 따라 "대기 중 / 진행 중 / 일시정지" 를 말한다
- [ ] 기록 목록의 한 줄이 "14시 5분, 51분, 통계 화면 다시 잡기" 처럼 **한 번에** 읽힌다 (네 조각으로 나뉘면 안 된다)
- [ ] 설정의 기본 시간에서 위아래 쓸어넘기기로 5분씩 조정된다
- [ ] 오늘 링과 주간 스트립이 숫자가 아니라 말로 읽힌다
