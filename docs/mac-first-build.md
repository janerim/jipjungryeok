...
- [ ] 권한 허용 후 세션 완료 시 `집중` 캘린더에 이벤트가 생긴다
- [ ] 이벤트 제목이 `🎯 집중 25분` / 중도 중지는 `🎯 집중 7분 (중단)` (실제 집중 시간 기준)
- [ ] 권한을 거부해도 앱이 정상 동작하고 세션은 저장된다
- [ ] 사용자가 `집중` 캘린더를 지워도 다음 세션에서 재생성된다
- [ ] **⚠️ 앱을 여러 번 껐다 켜도 `집중` 캘린더가 하나뿐이다**

마지막 항목이 이번 마일스톤의 최대 위험이다. 기획서가 **쓰기 전용 권한**을
쓰기로 했는데(§7), 그 상태에서 `calendar(withIdentifier:)` 와 `calendars(for:)` 가
막힐 수 있다. 막히면 매번 기존 캘린더를 못 찾아 새로 만들고 중복이 쌓인다.

중복이 생기면 `CalendarService.focusCalendar()` 를 손봐야 한다. 선택지는
전용 캘린더를 포기하고 `defaultCalendarForNewEvents` 로만 기록하거나,
전체 접근 권한(`requestFullAccessToEvents`)으로 바꾸는 것이다.
후자는 기획서의 프라이버시 판단을 뒤집는 것이라 결정이 필요하다.
