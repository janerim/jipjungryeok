...
2. **회고** — 최근 완료 세션 3건, 가로 3분할
   - 위에서부터: 날짜(`M월 d일`) / 집중 분(정수) / 종료 시각(`HH:mm`) / 메모 1줄
   - 메모가 없으면 그 줄은 비워 두되 자리는 남긴다. 세 칸의 높이가 어긋나 보이면 안 된다
   - 섹션 오른쪽의 **전체** 를 누르면 아래 기록 화면이 열린다
2-1. **기록** (회고에서 진입하는 시트) — 지난 세션 전체를 날짜별로 훑는다
   - 회고 카드가 "요즘 어땠나" 를 흘깃 보는 자리라면, 여기는 "그때 뭘 했더라" 를 찾아 들어가는 자리다. **메모가 주인공이다** — 다시 읽을 곳이 없으면 메모를 쓸 이유도 없다
   ...
   - **중도 중지한 세션도 포함한다.** 회고 카드는 완료된 것만 보여주지만 이력은 그 날 무엇을 했나의 기록이라 중간에 접은 것도 사실의 일부다
   - 날짜 판정은 회고·차트와 같이 `startAt` 기준. 세션이 0건이면 빈 상태를 그린다
   - 세션마다 카드를 두르지 않는다. 하루에 여러 건이면 테두리끼리 부딪혀 덩어리져 보인다
   - 일/주/월 전환 같은 모드는 두지 않는다. 기간별 합계는 위의 추이 그리드가 이미 답한다
3. **차트** — 일별 막대 그래프
