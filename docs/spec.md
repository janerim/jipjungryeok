...
| 완료 시 알림음 | **무음. 진동도 없다** (앱을 켜둔 채 사용하는 전제) |
...
2. 중앙 원형 다이얼 (90분 스케일)
3. 하단 큰 숫자 (남은 시간) — **이 앱의 시작·정지 버튼이다**
   - 예전에는 다이얼에서 손을 떼면 즉시 시작했다. 시간을 맞추다 손이 미끄러지면 원하지 않은 세션이 시작되고, 되돌리려면 길게 눌러 중지해야 했다. 시작은 의도한 행동이어야 한다
   - idle 이면 아래에 `탭하여 시작`, paused 면 `탭하여 계속` 을 한 줄 붙인다. 눌러야 시작한다는 것을 알 방법이 이것뿐이다. 돌아가는 중에는 비운다 — 그때는 숫자가 스스로 상태를 말한다
...
- 하단 숫자
  - **쉬고 있을 때**(다이얼을 맞추는 중): 분만 (`25`). 스냅이 1분 단위라 그 자리에서 초는 의미가 없다
  - **돌아갈 때**: `M:SS` 로 **매초 움직인다** (`24:59`)
  - 이전 판은 1분 이상일 때 올림한 분만 보여줬는데(25분 세션이 60초 동안 `25`), 부채꼴은 줄어드는데 주인공 숫자만 굳어 있어 타이머가 고장난 것처럼 보였다
...
| idle | 드래그 종료(손 뗌) | 시간만 확정. **시작하지 않는다** |
| idle | **하단 숫자 탭** | **카운트다운 시작** |
| running | 하단 숫자 탭 | 일시정지 |
| paused | 하단 숫자 탭 | 재개 |
| running | 다이얼 탭 | 일시정지 |
| paused | 다이얼 탭 | 재개 |
| paused | 다이얼 드래그 | 세션 취소 후 새 시간 설정 (기존 진행분은 §6 규칙대로 처리) |
| running/paused | 길게 누르기(0.6s) | 세션 중지 → idle (진동 없음) |
...
3. **완료 처리 시:**
   - **알림음도 진동도 사용하지 않는다.** 포그라운드면 화면이 이미 끝났다는 것을 말해 주므로 아무것도 하지 않고, 백그라운드면 무음 로컬 알림 배너만 (`content.sound = nil`)
   - 완료 햅틱은 넣었다가 뺐다. 집중이 끝난 순간 손목이나 주머니가 울리는 것은 "완료는 조용히" 와 어긋난다. 햅틱은 다이얼 스냅에만 남긴다 — 그건 1분 단위로 걸렸다는 신호라 없으면 화면을 계속 봐야 한다
...
- 프레임워크: **EventKit**
- 권한: 기본은 `requestWriteOnlyAccessToEvents()` (쓰기 전용). 기록 자체에는 이걸로 충분하고 심사·프라이버시에서 유리하다
- **캘린더를 고르는 기능만 전체 접근(`requestFullAccessToEvents()`)을 쓴다.**
  - 쓰기 전용 권한으로는 이벤트를 만들 수는 있어도 `calendars(for:)` 가 캘린더를 열거하지 못한다. 시뮬레이터에서는 로컬 캘린더가 보여서 되는 줄 알았는데, **실기기의 iCloud 계정에서는 빈 목록이 온다**
  - 그래서 기본값으로 요구하지 않는다. 설정에서 "캘린더 선택 허용" 을 누른 사람에게만 묻는다. 전체 접근은 사용자의 모든 일정을 읽을 수 있다는 뜻이라, 그게 필요 없는 사람에게까지 물어보지 않는다
  - 권한이 없으면 그 자리에 이유와 버튼을 보여준다. 목록만 비워 두면 "왜 안 나오지" 로 끝난다
- `Info.plist`: `NSCalendarsWriteOnlyAccessUsageDescription` + `NSCalendarsFullAccessUsageDescription`
  - 전체 접근 키가 없으면 요청하는 순간 앱이 죽는다
...
- [ ] 완료 시 소리도 진동도 없다. 백그라운드였다면 무음 배너만 뜬다
...
2. **무음 정책** — 앱을 켜둔 채 사용하는 전제다. 완료 시 소리도 진동도 주지 않고, 백그라운드에서는 무음 배너만 뜬다는 점을 감수한다.
