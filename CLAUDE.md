...
| M2 | 영속화 & 통계 | 완료 · 빌드 검증됨 |
| M3 | 알림 & 상태 복구 | 실기기 확인 완료 |
| M4 | 캘린더 연동 | 실기기 확인 완료 |
| M5 | 위젯 & Live Activity | 홈 위젯·Dynamic Island 확인 · 잠금화면 미확인 |
...
실기기(iPhone 17)에서 확인된 것: 다이얼·숫자 탭 조작, 상태 복구, 캘린더 목록과 기록,
백그라운드 무음 배너, 포그라운드 배너 미발생, 일시정지·중지 후 알림 취소, 버튼 피드백.

아직 못 본 것: 잠금화면 Live Activity, 잠금화면 위젯, 제스처 충돌(§4.0), 다크모드.
체크리스트는 mac-first-build.md 3.5·4단계.
