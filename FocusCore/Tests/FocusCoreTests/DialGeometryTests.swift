...
final class DialGeometryTests: XCTestCase {
    ...
    /// 한 바퀴가 90분이므로 시계 눈금과 분이 1:1로 맞지 않는다.
    /// 3시 방향이 15분이 아니라 22.5분이다.
    func testQuarterTurnsGoClockwise() {
        assertPoint(
            DialGeometry.point(atMinute: 22.5, radius: radius, center: center),
            270, 170, "1/4 바퀴(22.5분)는 3시 방향(오른쪽)"
        )
        assertPoint(
            DialGeometry.point(atMinute: 45, radius: radius, center: center),
            170, 270, "반 바퀴(45분)는 6시 방향(아래)"
        )
        assertPoint(
            DialGeometry.point(atMinute: 67.5, radius: radius, center: center),
            70, 170, "3/4 바퀴(67.5분)는 9시 방향(왼쪽)"
        )
    }
    ...
    func testTouchPositionsMapToExpectedMinutes() {
        ...
        XCTAssertEqual(
            ...
            68, "왼쪽 터치는 67.5분 → 반올림 68분"
        )
    }

    /// point() 로 만든 좌표를 다시 분으로 되돌리면 같은 값이 나와야 한다.
    func testPointAndSnapRoundTrip() {
        for minute in [1, 7, 13, 22, 29, 38, 44, 51, 59, 67, 75, 83, 89] {
    ...
    // MARK: - 12시 경계 (§4.1 최소 1분 / 최대 90분)

    /// 88분 자리에서 12시를 넘어 2분 자리로 끌면 90분에서 붙잡혀야 한다.
    ...
    func testDraggingForwardPastTwelveClampsToMax() {
        ...
        XCTAssertEqual(DialGeometry.snappedMinutes(at: p, center: center, previous: 88), 90)
    }

    /// 반대 방향으로 넘으면 1분에서 붙잡힌다.
    func testDraggingBackwardPastTwelveClampsToMin() {
        let p = DialGeometry.point(atMinute: 88, radius: radius, center: center)
        XCTAssertEqual(DialGeometry.snappedMinutes(at: p, center: center, previous: 2), 1)
    }

    /// 경계를 넘지 않는 평범한 이동은 그대로 통과한다.
    /// 가드 기준은 한 바퀴의 1/4·3/4 이므로 90분에서는 22.5분과 67.5분이다.
    func testNormalMovementIsNotClamped() {
        let cases: [(from: Int, to: Int)] = [(30, 35), (88, 89), (3, 2), (20, 19), (60, 65)]
        for c in cases {
    ...
    /// 12시 정각은 0분이 아니라 최대값으로 읽는다. 0분짜리 세션은 존재하지 않는다.
    func testExactTwelveReadsAsMaximumNotZero() {
        let top = CGPoint(x: center.x, y: center.y - radius)
        XCTAssertEqual(DialGeometry.snappedMinutes(at: top, center: center, previous: 89), 90)
    }

    /// 스케일은 `TimerEngine.maximumMinutes` 하나에서 나온다.
    /// 최대 시간을 바꿀 때 이 파일의 기대값만 고치면 되도록 못박아 둔다.
    func testScaleFollowsMaximumMinutes() {
        XCTAssertEqual(TimerEngine.maximumMinutes, 90, "한 바퀴 = 90분 (§4.1)")
        let halfTurn = DialGeometry.point(
            atMinute: Double(TimerEngine.maximumMinutes) / 2,
            radius: radius,
            center: center
        )
        assertPoint(halfTurn, 170, 270, "반 바퀴는 언제나 6시 방향이어야 한다")
    }

    // MARK: - 중심 데드존

    /// 정확히 중심을 누르면 atan2(0, 0) 이 0 을 돌려줘 3시 방향 값으로 튄다.
